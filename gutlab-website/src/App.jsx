...
import { Sparkles, Gift, Coffee, ShoppingBag, Stars, Heart, CupSoda } from 'lucide-react';
import { useState } from 'react';

function ProductImage({ src, alt, className, fallbackLabel }) {
  const [broken, setBroken] = useState(false);

  if (broken) {
    return (
      <div className={`flex items-center justify-center bg-white/70 text-center text-sm font-semibold text-stone-500 ${className}`}>
        <div>
          <div className="mx-auto mb-2 h-12 w-12 rounded-full bg-orange-100" />
          <p>{fallbackLabel}</p>
          <p className="mt-1 text-xs">Add the real image in /public</p>
        </div>
      </div>
    );
  }
...
export default function App() {
  ...
  const floating = ['top-24 left-6', 'top-32 right-10', 'bottom-16 left-10', 'bottom-24 right-8'];
  const whatsapp = 'https://wa.me/6584067441?text=Hi%20Gut%20Lab%20by%20Xaels%2C%20I%20would%20like%20to%20place%20an%20order.';
  const instagram = 'https://www.instagram.com/xaelsbakes?igsh=MTBicDlwZ3piMXdyNg==';
  const tiktok = 'https://vt.tiktok.com/ZSH2SPbE4/?page=TikTokShop';
        ...
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4 lg:px-10">
          <div className="flex items-center gap-3">
            <div className="flex h-14 w-14 items-center justify-center overflow-hidden rounded-[1.4rem] border border-amber-100 bg-white shadow-[0_8px_25px_rgba(0,0,0,0.08)]">
              <ProductImage src="/logo.png" alt="Gut Lab by Xaels logo" className="h-full w-full object-contain" fallbackLabel="Logo" />
            </div>
            <div>
              <p className="text-lg font-black tracking-tight text-[#21451f]">Gut Lab by Xaels</p>
              <p className="text-xs text-stone-500">Joyful everyday nourishment</p>
            </div>
          </div>

          <nav className="hidden gap-6 text-sm font-medium text-stone-600 md:flex">
            <a href="#products" className="transition hover:text-[#21451f]">Products</a>
            <a href="#bundles" className="transition hover:text-[#21451f]">Bundles</a>
            <a href="#daily-use" className="transition hover:text-[#21451f]">Daily Use</a>
            <a href="#contact" className="transition hover:text-[#21451f]">Order</a>
          </nav>
        </div>
      </header>
      ...
      <main>
        <section className="relative mx-auto max-w-7xl px-6 py-16 lg:px-10 lg:py-24">
          <div className="relative overflow-hidden rounded-[2.5rem] border border-white/60 bg-[linear-gradient(135deg,#fff8ef_0%,#fff1de_45%,#ffe6c9_100%)] p-8 shadow-[0_20px_80px_rgba(164,112,42,0.18)] lg:p-12">
            ...
            {floating.map((pos, index) => (
              ...
              >
                {index % 2 === 0 ? <Sparkles className="h-6 w-6 text-amber-500" /> : <Stars className="h-6 w-6 text-orange-400" />}
              </motion.div>
            ))}
            ...
            <div className="relative grid gap-12 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">
              <div>
                <div className="mb-4 inline-flex items-center gap-2 rounded-full border border-amber-200 bg-white/75 px-4 py-2 text-xs font-bold uppercase tracking-[0.18em] text-[#9b6918] shadow-sm">
                  <Sparkles className="h-4 w-4" /> Selling collection
                </div>
                ...
                <p className="mt-5 max-w-xl text-base leading-7 text-stone-600 sm:text-lg">
                  Gut Lab by Xaels brings playful, premium, story-rich packaging together with products people can enjoy every day — from breakfast toast to warm cocoa moments.
                </p>

                <div className="mt-8 flex flex-wrap gap-3">
                  <a href="#products" className="rounded-2xl bg-[#21451f] px-6 py-3 text-sm font-bold text-white shadow-[0_12px_30px_rgba(33,69,31,0.25)] transition hover:-translate-y-1">
                    Shop the range
                  </a>
                  <a href="#bundles" className="rounded-2xl border border-amber-200 bg-white px-6 py-3 text-sm font-bold text-[#21451f] shadow-sm transition hover:-translate-y-1">
                    See bundles
                  </a>
                </div>

                <div className="mt-8 flex flex-wrap gap-3 text-sm">
                  {['No refined sugar', 'No preservatives', 'Gift-worthy packaging', 'Made for repeat buying'].map((item) => (
                    <span key={item} className="rounded-full border border-white/70 bg-white/75 px-4 py-2 font-medium text-stone-700 shadow-sm backdrop-blur">
                      {item}
                    </span>
                  ))}
                </div>
              ...
              <div className="grid gap-4">
                ...
                >
                  <div className="sm:row-span-2 overflow-hidden rounded-[2rem] border border-white/70 bg-white/70 p-4 shadow-[0_18px_50px_rgba(0,0,0,0.08)] backdrop-blur">
                    <ProductImage src="/healthy_spread.jpg" alt="Healthy Spread" className="h-full w-full rounded-[1.4rem] object-cover" fallbackLabel="Healthy Spread" />
                  </div>
                  <div className="overflow-hidden rounded-[2rem] border border-white/70 bg-white/70 p-4 shadow-[0_18px_50px_rgba(0,0,0,0.08)] backdrop-blur">
                    <ProductImage src="/coffee_spread.jpg" alt="Coffee Reimagined" className="h-full w-full rounded-[1.4rem] object-cover" fallbackLabel="Coffee Spread" />
                  </div>
                  <div className="overflow-hidden rounded-[2rem] border border-white/70 bg-white/70 p-4 shadow-[0_18px_50px_rgba(0,0,0,0.08)] backdrop-blur">
                    <ProductImage src="/cocoa_mix.jpg" alt="Cocoa Nut Mix" className="h-full w-full rounded-[1.4rem] object-cover" fallbackLabel="Cocoa Nut Mix" />
                  </div>
                </motion.div>
              </div>
            </div>
        ...
        <section className="mx-auto max-w-7xl px-6 pb-8 lg:px-10">
          <div className="grid gap-4 md:grid-cols-4">
            {[
              ...
              <div key={title} className="rounded-[1.8rem] border border-white/70 bg-white/75 p-5 shadow-[0_12px_35px_rgba(0,0,0,0.05)] backdrop-blur">
        ...
        <section id="products" className="mx-auto max-w-7xl px-6 py-16 lg:px-10">
          <div className="mb-10 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div className="max-w-2xl">
              <p className="text-sm font-bold uppercase tracking-[0.18em] text-[#9b6918]">Shop products</p>
              <h2 className="mt-3 text-3xl font-black tracking-tight text-[#21451f] sm:text-4xl">A magical-looking storefront that still feels like your brand</h2>
              <p className="mt-4 text-base leading-7 text-stone-600">
                Original playful 3D-inspired layouts, soft gradients, floating accents, and rounded cards create an attractive storybook-style shopping feel.
              </p>
            </div>
            <a href="#contact" className="inline-flex rounded-2xl border border-amber-200 bg-white px-5 py-3 text-sm font-bold text-[#21451f] shadow-sm">
              Quick order
            </a>
          </div>
                ...
                >
                  <div className={`relative bg-gradient-to-br ${product.glow} p-4`}>
                    ...
                    <div className="overflow-hidden rounded-[1.7rem] border border-white/70 bg-white/80 p-3 shadow-lg">
                      <ProductImage src={product.image} alt={product.name} className="h-72 w-full rounded-[1.25rem] object-cover transition duration-500 group-hover:scale-[1.03]" fallbackLabel={product.name} />
                    </div>
                  </div>
                  ...
                  <div className="p-6">
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        <h3 className="text-2xl font-black tracking-tight text-[#21451f]">{product.name}</h3>
                    ...
                    <p className="mt-3 text-sm font-bold text-[#9b6918]">{product.tagline}</p>
                    <p className="mt-4 text-sm leading-6 text-stone-600">{product.description}</p>

                    <div className="mt-5 flex flex-wrap gap-2">
                      {product.notes.map((item) => (
                        <span key={item} className="rounded-full border border-orange-100 bg-orange-50 px-3 py-1 text-xs font-medium text-stone-700">
                          {item}
                        </span>
                      ))}
                    </div>
                    ...
                    <div className="mt-5 rounded-[1.4rem] bg-stone-50 p-4">
                      <p className="text-sm font-bold text-[#21451f]">How to enjoy</p>
                    ...
                    <a
                      href={`https://wa.me/6584067441?text=${encodeURIComponent(`Hi Gut Lab by Xaels, I would like to order ${product.shortName}.`)}`}
        ...
        <section id="bundles" className="mx-auto max-w-7xl px-6 py-8 lg:px-10 lg:py-16">
          <div className="overflow-hidden rounded-[2.4rem] border border-white/70 bg-[linear-gradient(135deg,#21451f_0%,#315d2f_100%)] p-8 text-white shadow-[0_25px_70px_rgba(33,69,31,0.25)] lg:p-10">
            <div className="mb-8 max-w-2xl">
              <p className="text-sm font-bold uppercase tracking-[0.18em] text-amber-200">Bundles</p>
              <h2 className="mt-3 text-3xl font-black tracking-tight sm:text-4xl">Cute bundle cards help the page feel richer and more shoppable</h2>
              <p className="mt-4 text-sm leading-7 text-stone-200">
                Bundle options make the storefront more attractive and guide customers toward larger basket sizes.
              </p>
            </div>
                ...
                return (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <p className="inline-flex rounded-full bg-white/15 px-3 py-1 text-xs font-bold text-amber-100">{bundle.badge}</p>
                      <div className="rounded-full bg-white/15 p-2">
                        <Icon className="h-5 w-5 text-amber-100" />
                      </div>
                    </div>
                    <h3 className="mt-4 text-2xl font-black tracking-tight">{bundle.title}</h3>
                    <p className="mt-2 text-3xl font-black text-amber-200">{bundle.price}</p>
                    <p className="mt-3 text-sm leading-6 text-stone-100">{bundle.detail}</p>
                    <a
                      href={`https://wa.me/6584067441?text=${encodeURIComponent(`Hi Gut Lab by Xaels, I would like to order the ${bundle.title}.`)}`}
        ...
        <section id="daily-use" className="mx-auto max-w-7xl px-6 py-16 lg:px-10">
          <div className="grid gap-8 lg:grid-cols-[0.95fr_1.05fr] lg:items-start">
            <div>
              <p className="text-sm font-bold uppercase tracking-[0.18em] text-[#9b6918]">Daily use</p>
              <h2 className="mt-3 text-3xl font-black tracking-tight text-[#21451f] sm:text-4xl">The page explains why people would buy again</h2>
              <p className="mt-4 text-base leading-7 text-stone-600">
                A good selling website needs to show not just what the product is, but how it fits real life. This section gives your products repeat-purchase logic.
              </p>
            </div>
        ...
        <section className="mx-auto max-w-7xl px-6 pb-16 lg:px-10">
          <div className="grid gap-6 md:grid-cols-3">
            {reasons.map((reason) => (
              <div key={reason.title} className="rounded-[2rem] border border-white/70 bg-white/80 p-6 shadow-[0_18px_45px_rgba(0,0,0,0.05)] backdrop-blur">
                <h3 className="text-xl font-black tracking-tight text-[#21451f]">{reason.title}</h3>
        ...
        <section id="contact" className="mx-auto max-w-7xl px-6 pb-20 lg:px-10">
          <div className="overflow-hidden rounded-[2.5rem] border border-white/70 bg-[linear-gradient(135deg,#fff7eb_0%,#ffeccf_100%)] p-8 shadow-[0_20px_70px_rgba(197,138,47,0.18)] lg:p-10">
            <div className="grid gap-8 lg:grid-cols-[1fr_auto] lg:items-center">
              <div>
                <p className="text-sm font-bold uppercase tracking-[0.18em] text-[#9b6918]">Ready to order?</p>
                <h2 className="mt-3 text-3xl font-black tracking-tight text-[#21451f] sm:text-4xl">Message us directly and turn this page into a real sales channel</h2>
                <p className="mt-4 max-w-2xl text-base leading-7 text-stone-600">
                  Replace the placeholder image files with your real product photos in the public folder and the site is ready for launch.
                </p>
              </div>
              <div className="flex flex-col gap-3">
                <a href={whatsapp} className="inline-flex items-center justify-center rounded-2xl bg-[#21451f] px-6 py-4 text-sm font-bold text-white shadow-[0_14px_30px_rgba(33,69,31,0.25)] transition hover:-translate-y-1">
                  Order via WhatsApp
                </a>
                <a href={instagram} className="inline-flex items-center justify-center rounded-2xl border border-amber-200 bg-white px-6 py-4 text-sm font-bold text-[#21451f] shadow-sm transition hover:-translate-y-1">
                  Visit Instagram
                </a>
                <a href={tiktok} className="inline-flex items-center justify-center rounded-2xl border border-amber-200 bg-white px-6 py-4 text-sm font-bold text-[#21451f] shadow-sm transition hover:-translate-y-1">
                  Visit TikTok Shop
                </a>
              </div>
            </div>
      ...
      <footer className="border-t border-white/60 bg-white/70 backdrop-blur">
        <div className="mx-auto grid max-w-7xl gap-8 px-6 py-12 lg:grid-cols-2 lg:px-10">
          <div className="flex items-start gap-4">
            ...
            <div>
              <p className="text-2xl font-black tracking-tight text-[#21451f]">Gut Lab by Xaels</p>
              <p className="mt-3 max-w-md text-sm leading-6 text-stone-600">A cheerful, modern storefront for science-led everyday nourishment.</p>
            </div>
          </div>
